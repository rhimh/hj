...
return send(msg.chat_id,msg.id,"⌔️ هذا الامر يخص المدير")
end
Redis:del(Fast..msg.chat_id..'chat_lock:lock')
send(msg.chat_id,msg.id,"⌔️ تم تفعيل قفل الدردشه التلقائي")
end
...
send(msg.chat_id,msg.id,"⌔️ تم تعطيل قفل الدردشه التلقائي")
end
...
local txx = "⌔️ القفل التلقائي { "..state.." }\n⌔️ الوقت الان "..current_time.."\n\n⌔️ وقف بدايه القفل » "..lock_time.."\n⌔️ وقت نهايه القفل » "..on_time
...
return send(msg.chat_id,msg.id,"⌔️ تم الغاء الامر")
end
...
return send(msg.chat_id,msg.id,"⌔️ تم حفظ وقف القفل الساعه "..text.."\n ارسل الان وقت التفعيل ")
else
return send(msg.chat_id,msg.id,"⌔️ لقد ارسلت وقت خاطئ")
end
else
return send(msg.chat_id,msg.id,"⌔️ لقد ارسلت وقت خاطئ")
end
...
return send(msg.chat_id,msg.id,"⌔️ تم حفظ وقف الفتح الساعه "..lock_time.."\n")
else
return send(msg.chat_id,msg.id,"⌔️ لقد ارسلت وقت خاطئ")
end
else
...
send(msg.chat_id,msg.id,"⌔️ ارسل الان الوقت بنظام 24 ساعه")
end
...
Redis:del(Fast..msg.chat_id..'time:chat:lock')
send(msg.chat_id,0,"⌔️ تم قفل الدردشه تلقائيا")
end
end
if Redis:get(Fast..msg.chat_id..'time:chat:on') then
local current_time = https.request("https://xnxx.fastbots.ml/apies/auto.php")
...
Redis:del(Fast..msg.chat_id..'time:chat:on')
send(msg.chat_id,0,"⌔️ تم فتح الدردشه تلقائيا")
end
end
end
